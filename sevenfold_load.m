...
load('50_comb_7_loads')
loads_combination = y;

load('10_comb_trig_7_loads')
trig = y;
...
for i=1:length(loads_combination)
    ...
    sim6 = loads_combination(i,6);
    sim7 = loads_combination(i,7);
    ...
    for j=1:length(trig)
        ...
        c7 = trigger_label(c7);
                ...
                off_1 = on_1 + (cycle*700);
                off_2 = on_2 + (cycle*700);
                off_3 = on_3 + (cycle*700);
                off_4 = on_4 + (cycle*700);
                off_5 = on_5 + (cycle*700);
                off_6 = on_6 + (cycle*700);
                off_7 = on_7 + (cycle*700);
                ...
                set_param(sprintf('%s/%s',simulation,load_blocks(sim6)),'commented','off')
                set_param(sprintf('%s/%s',simulation,load_blocks(sim7)),'commented','off')
                ...
                set_param(sprintf('%s/%s',simulation,on_off_blocks(sim6)),'commented','off');
                set_param(sprintf('%s/%s',simulation,on_off_blocks(sim7)),'commented','off');
                ...
                current = decimate(current,70);
                voltage = decimate(voltage,70);        

                label = string(simu_blocks(sim1) + '+' + simu_blocks(sim2) + '+' + simu_blocks(sim3)+ '+' + simu_blocks(sim4)+ '+' + simu_blocks(sim5)+ '+' + simu_blocks(sim6)+ '+' + simu_blocks(sim7));
                trigger_angle = [(c1*60*180);(c2*60*180); (c3*60*180); (c4*60*180); (c5*60*180); (c6*60*180); (c7*60*180)] ;
                ...
                N_on_6 = on_6*Ts/decimation;
                N_on_7 = on_7*Ts/decimation;
                ...
                N_off_6 = off_6*Ts/decimation;
                N_off_7 = off_7*Ts/decimation;
                ...
                set_param(sprintf('%s/%s',simulation,load_blocks(sim6)),'commented','on');
                set_param(sprintf('%s/%s',simulation,load_blocks(sim7)),'commented','on');
                ...
                set_param(sprintf('%s/%s',simulation,on_off_blocks(sim6)),'commented','on');
                set_param(sprintf('%s/%s',simulation,on_off_blocks(sim7)),'commented','on');
                ...
                ativation_index(sim6)=1;
                ativation_index(sim7)=1;
