...
load('30_comb_3_loads')
loads_combination = y;


load('5_comb_trig_3_loads')
trig = y;
...
for i=1:length(loads_combination)
    ...
    for j=1:length(trig)
        ...
        c3 = trigger_label(c3);
                ...
                off_1 = on_1 + (cycle*300);
                off_2 = on_2 + (cycle*300);
                off_3 = on_3 + (cycle*300);
                ...
                label = string(simu_blocks(sim1) + '+' + simu_blocks(sim2) + '+' + simu_blocks(sim3));
                trigger_angle = [(c1*60*180);(c2*60*180); (c3*60*180)] ;
